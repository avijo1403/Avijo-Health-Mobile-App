...
export default function ConfirmBooking({ navigation, route }) {
    ...
    const fetchData = async()=>{
        try{
            const response = await fetch(`${BaseUrl2}/user/getDoctorConsultationById/${id}`);
            const json = await response.json();
            setSingleData(json);
            console.log('json:', json.consultation);
        }catch(e){
        ...
        <View style={styles.container}>
                ...
                <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginRight: '20%' }}>
                    <TouchableOpacity onPress={()=>navigation.navigate('Reviews',{id:singleData?.consultation?._id})} style={{ width: '70%', marginTop: '5%', flexDirection: 'row' }}>
                        ...
                        <View style={{ marginLeft: '5%', width: '70%' }}>
                            <Text style={{ fontFamily: 'Gilroy-SemiBold', fontSize: 12, color: colors.blue }}>{singleData?.consultation?.name}</Text>
                            <Text style={{ fontFamily: 'Gilroy-Medium', fontSize: 12, color: colors.darkGrey, paddingTop: '3%' }}>{singleData?.consultation?.speciality}</Text>
                            <View style={{ flexDirection: 'row', marginTop: '1%', width: '90%', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <Text style={{ fontSize: 9, fontFamily: 'Gilroy-Medium', color: colors.darkGrey }}>fees. <Text style={{ fontSize: 9, fontFamily: 'Gilroy-SemiBold', color: colors.black }}>${singleData?.consultation?.price}</Text></Text>
                            </View>
                            <View style={{ flexDirection: 'row', marginTop: '1%', width: '90%', alignItems: 'center', justifyContent: 'space-between' }}>
           ...
           <View style={{width:'100%'}}>
                ...
                <View style={{ width: '100%', alignItems: 'center', marginTop: '5%', marginBottom: '5%' }}>
                    <Button1 Text="Confirm booking" onPress={() => navigation.navigate('AppointmentConfirm')} />
                </View>
            </ScrollView>
        </View>
    );
